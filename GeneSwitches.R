...
option_list = list(
    ...
    make_option( c("--monocle", "-m"), type = "character", 
                 help = "the monocle object saved as R object in RDS format."),
...
if (is.null(opt$states)){
    num=1
    branch=list(as.integer(unique(phenoData(monocle)@data[order(phenoData(monocle)$Pseudotime),"State"])))
} else {
...
for ( i in 1:num){
    ###### prepare
    sce[[i]] <- convert_monocle2(monocle2_obj = monocle, 
                               states = branch[[i]] , expdata = logexpdata)
    pdf(file.path(output_dir,paste0(i,"hist.pdf")))
    ...
    write.table(all_data ,file.path(output_dir,paste0("all_switching_genes_for_branch_State",paste0(branch[[i]],collapse="-"),".xls")),quote=F, sep="\t",row.names=F)       
    sig_data=all_data %>% filter(zerop_gene < 0.9 ,pseudoR2s > 0.2)
    write.table(sig_data ,file.path(output_dir,paste0("sig_switching_genes_for_branch_State",paste0(branch[[i]],collapse="-"),"_R2-0.2.xls")),quote=F, sep="\t",row.names=F)       
    ...
    ggsave(file.path(output_dir,paste0("timeline_for_branch_S",paste0(branch[[i]],collapse="-"),".pdf")),timeline)
    ggsave(file.path(output_dir,paste0("timeline_for_branch_S",paste0(branch[[i]],collapse="-"),".png")),timeline,dpi=1000)
    ...
    for ( gmt  in names(gmt_file)){
        ...
        if (nrow(switch_pw_reduce) > 0) {
            ...
            ggsave(file.path(output_dir,paste0("enrichment/",gmt,"_top10_significant_pathways_for_branch_S",paste0(branch[[i]],collapse="-"),".pdf")),pathway,width=14)
            ggsave(file.path(output_dir,paste0("enrichment/",gmt,"_top10_significant_pathways_for_branch_S",paste0(branch[[i]],collapse="-"),".png")),pathway,width=14)
        } else {
...
if (num == 2 ) {
    ...
    ggsave(file.path(output_dir,"distinct_switching_genes_between_two_branches.pdf"),diff)
    ggsave(file.path(output_dir,"distinct_switching_genes_between_two_branches.png"),diff)
    ...
    diff_scaled= plot_timeline_ggplot(sg_disgs_scaled, timedata = 1:100, color_by = "Paths", 
                         iffulltml = FALSE, txtsize = 3)
    ggsave(file.path(output_dir,"distinct_switching_genes_between_two_branches_scaled.pdf"),diff_scaled)
    ggsave(file.path(output_dir,"distinct_switching_genes_between_two_branches_scaled.png"),diff_scaled)
}
